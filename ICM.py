...
def partition_data(N_partition, t_data, j_data, V_data, idx_in=0, DVmin = 2.e-4, idx_switch= None, GITT=False, useGITTperiods=False):
    ...
    if GITT and useGITTperiods:
        ...
        mask_threshold = np.r_[False, (np.diff(V_data)) >= DVmin]
...
def Rsquared(c_values, D_values, Ueq, c0, j_int, t_data, V_data, Vardata, N, dr, M_inv, r2mid, dt_max_global, idx_in=0, events=False):    
    sorted_idx = np.argsort(c_values)
    sorted_c_values = c_values[sorted_idx]
    sorted_D_values = D_values[sorted_idx]
...
def gradient_descent(c_knots, D_knots, Ueq, c0, j_int, t_data, V_data, x_data, dt_max_global
    ...
    R2_iter = Rsquared(c_knots, D_knots, Ueq, c0, j_int, t_data, V_data, VarDV_data, N, dr, M_inv, r2mid, dt_max_global)
    R2_iter_last = -np.inf
    ...
    while N_iter < N_iter_max+  1:
        ...
        R2_iter, R2_iter_last = Rsquared(*R2_para) , R2_iter
        
        
        R2list += [R2_iter,]
        D_iter_list += [D_iter.copy(),]
        
        f"R^2 = {R2_iter:.4f}  [Iteration # {N_iter}]"
